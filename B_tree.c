...
void find_left_depth(char* root);
        ...
        while (true) {
            ...
            if ((store = custom_strcmp(&tmp, str, tmp_length)) >= 0) {
                ...
                if (store == 0) {
                    ...
                    /* Checking */
                    //printf("Key is found, quiting the function...\n");
                    //memcpy(array_store, prev + sizeof(char*) + sizeof(int), tmp_length + 1);
                    //printf("Key is %s, string is %s\n", array_store, str);
...
inline size_t get_init_param_offset(void) {
...
inline size_t skip_initial_parameters(char** node_ptr) {
    /* Sanity Check */
    //if (!node_ptr) {
    //    fprintf(stderr, "Null pointer...\n");
    //    return 0;
    ...
    size_t tmp = get_init_param_offset();
...
void find_left_depth(char* root) {
    char* current = root;
    while(current != NULL) {
        current = *(char**)(current + get_init_param_offset());
        left_tree_height++;
    }
}
